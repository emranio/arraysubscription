...
use ArraySubscriptionVendors\is_a;
use ArraySubscriptionVendors\ltrim;
use ArraySubscriptionVendors\pathinfo;
use ArraySubscriptionVendors\strtolower;
use ArraySubscriptionVendors\PATHINFO_EXTENSION;
use ArraySubscriptionVendors\JG\Config\Parsers\ParserInterface;
use ArraySubscriptionVendors\JG\Config\Exceptions\InvalidParserException;
/**
 * ConfigParserFactory
 * 
 * Factory class for creating and managing configuration parsers. 
...
class ConfigParserFactory
{
     ...
     */
    private static array $configParsers = ['conf' => \ArraySubscriptionVendors\JG\Config\Parsers\ConfParser::class, 'ini' => \ArraySubscriptionVendors\JG\Config\Parsers\IniParser::class, 'json' => \ArraySubscriptionVendors\JG\Config\Parsers\JsonParser::class, 'php' => \ArraySubscriptionVendors\JG\Config\Parsers\PhpParser::class, 'xml' => \ArraySubscriptionVendors\JG\Config\Parsers\XmlParser::class, 'yaml' => \ArraySubscriptionVendors\JG\Config\Parsers\YamlParser::class, 'yml' => \ArraySubscriptionVendors\JG\Config\Parsers\YamlParser::class];
    /**
    ...
    public static function createParser(string $filePath): ParserInterface
    {
        ...
        if ($parserClass && is_a($parserClass, ParserInterface::class, \true)) {
            return new $parserClass();
        }
        throw new InvalidParserException("No suitable parser found for file extension: .{$extension}. File: {$filePath}");
    }
    /**
    ...
    public static function registerParser(string $extension, string $parserClass): void
    {
        ...
        if (!is_a($parserClass, ParserInterface::class, \true)) {
            throw new InvalidParserException("Parser class '{$parserClass}' must implement ParserInterface.");
        }
