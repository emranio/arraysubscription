...
class PhpParser implements ParserInterface
{
    ...
    public function parse(string $filePath): array
    {
        if (!is_file($filePath) || !is_readable($filePath)) {
            throw new ConfigParseException(sprintf("Failed to parse PHP configuration file. File is not readable: %s", $filePath));
        }
        $data = require $filePath;
        if (!is_array($data)) {
            throw new ConfigParseException(sprintf("PHP configuration file must return an array. Error in file: %s", $filePath));
        }
        return $data;
    }
}
