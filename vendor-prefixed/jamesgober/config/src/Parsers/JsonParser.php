...
use ArraySubscriptionVendors\JSON_ERROR_NONE;
use ArraySubscriptionVendors\json_last_error;
use ArraySubscriptionVendors\file_get_contents;
use ArraySubscriptionVendors\json_last_error_msg;
...
class JsonParser implements ParserInterface
{
    ...
    public function parse(string $filePath): array
    {
        if (!is_file($filePath) || !is_readable($filePath)) {
            throw new ConfigParseException(sprintf("JSON file not found or unreadable: %s", $filePath));
        }
        $json = file_get_contents($filePath);
        if ($json === \false) {
            throw new ConfigParseException(sprintf("Failed to read JSON file: %s", $filePath));
        }
        $data = json_decode($json, \true);
        if (json_last_error() !== \JSON_ERROR_NONE) {
            throw new ConfigParseException(sprintf("Invalid JSON in file: %s. Error: %s", $filePath, json_last_error_msg()));
        }
        return $data ?? [];
    }
}
