...
use ArraySubscriptionVendors\is_numeric;
use ArraySubscriptionVendors\is_readable;
use SimpleXMLElement;
use ArraySubscriptionVendors\libxml_get_errors;
use ArraySubscriptionVendors\libxml_clear_errors;
use ArraySubscriptionVendors\simplexml_load_file;
use ArraySubscriptionVendors\libxml_use_internal_errors;
...
class XmlParser implements ParserInterface
{
    ...
    public function parse(string $filePath): array
    {
        if (!is_file($filePath) || !is_readable($filePath)) {
            throw new ConfigParseException(sprintf("XML file not found or unreadable: %s", $filePath));
        }
    ...
    protected function handleXmlErrors(?string $filePath): void
    {
        ...
        throw new ConfigParseException(sprintf("XML parsing error in file '%s': %s on line %d", $filePath ?? 'unknown', $errorMessage, $latestError->line ?? 0));
    }
    /**
