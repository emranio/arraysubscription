...
use ArraySubscriptionVendors\parse_ini_file;
use ArraySubscriptionVendors\INI_SCANNER_TYPED;
...
class IniParser implements ParserInterface
{
    ...
    public function parse(string $filePath): array
    {
        if (!is_file($filePath) || !is_readable($filePath)) {
            throw new ConfigParseException(sprintf("INI file not found or unreadable: %s", $filePath));
        }
        $data = parse_ini_file($filePath, \true, \INI_SCANNER_TYPED);
        if ($data === \false) {
            throw new ConfigParseException(sprintf("Failed to parse INI configuration file: %s", $filePath));
        }
        return $data;
    }
}
