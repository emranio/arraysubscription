...
namespace ArraySubscriptionVendors\JG\Config;

use ArraySubscriptionVendors\ltrim;
use ArraySubscriptionVendors\mkdir;
use ArraySubscriptionVendors\rtrim;
use ArraySubscriptionVendors\is_dir;
use ArraySubscriptionVendors\unlink;
use ArraySubscriptionVendors\is_file;
use ArraySubscriptionVendors\is_array;
use ArraySubscriptionVendors\strtolower;
use ArraySubscriptionVendors\array_merge;
use ArraySubscriptionVendors\array_shift;
use ArraySubscriptionVendors\json_decode;
use ArraySubscriptionVendors\json_encode;
use ArraySubscriptionVendors\preg_replace;
use ArraySubscriptionVendors\array_key_exists;
use ArraySubscriptionVendors\file_put_contents;
use ArraySubscriptionVendors\JSON_PRETTY_PRINT;
use ArraySubscriptionVendors\JSON_THROW_ON_ERROR;
use ArraySubscriptionVendors\JG\Config\ConfigParserFactory;
use ArraySubscriptionVendors\Psr\Http\Message\StreamInterface;
use ArraySubscriptionVendors\JG\Config\Exceptions\ConfigException;
/**
...
class Config
{
     ...
     */
    public function __construct(?string $configPath = null, bool $flattenConfig = \true)
    {
    ...
    private function flattenArray(array $data, string $baseKey): array
    {
        ...
        while ($stack) {
            ...
            if ($currentDepth >= $this->maxDepth) {
                // Correctly handle "max depth exceeded"
    ...
    {
        $data = json_decode($stream->getContents(), \true, 512, \JSON_THROW_ON_ERROR);
        if (!is_array($data)) {
    ...
    public function saveCache(string $filePath, int $expires = 0): bool
    {
        ...
        $data = ['config' => $this->config, 'groups' => $this->groups, 'expires' => $expires ? time() + $expires : 0];
    ...
    public function loadCache(string $filePath): bool
    {
        ...
        try {
            $data = json_decode(file_get_contents($filePath), \true, 512, \JSON_THROW_ON_ERROR);
        } catch (\JsonException $e) {
            unlink($filePath);
            // Cleanup malformed cache
            return \false;
        }
